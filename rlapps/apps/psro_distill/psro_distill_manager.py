...
from rlapps.algos.psro_distill.manager import (
...
def launch_manager(
    scenario: DistilledPSROScenario,
    psro_port: int,
    eval_port: int,
    block: bool = True,
    include_evals: bool = True,
) -> PSRODistillManagerWithServer:
    if not isinstance(scenario, DistilledPSROScenario):
        raise TypeError(
            f"Only instances of {DistilledPSROScenario} can be used here. {scenario.name} is a {type(scenario)}."
        )
    ...
    if scenario.calc_exploitability_for_openspiel_env:
        ...
        def get_manager_logger(_manager: PSRODistillManager):
    ...
    print(
        "Launched PSRODistill Manager with server. ray_head_address={}".format(
            ray_head_address
        )
    )
...
if __name__ == "__main__":
    ...
    scenario: DistilledPSROScenario = scenario_catalog.get(scenario_name=scenario_name)
